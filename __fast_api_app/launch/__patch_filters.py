...
"""
Добавление jsonb операторов в ОРМ
"""


class JsonbFounder(BasicCriterion):
    """
    Класс благодаря которому всё работает.
    Мы задаем любой оператор в условие where
    """

    def __init__(self, left, right, alias=None, operation='@>') -> None:
        """
        Возможность выбирать свой оператор
        """
        super().__init__(f" {operation} ", left, right, alias=alias)

    def get_sql(self, quote_char='"', with_alias=False, **kwargs):
        """
        Обязательный метод. Указывает как именно вернуть sql
        """
...
def jsonb_operator_include(field: Term, value: Any) -> Criterion:
    """
    Получить json объект, содержащий другой, т.е. «являющийся подмножеством»
    """
    return JsonbFounder(field, field.wrap_constant(value), operation='@>')


def jsonb_operator_match_or_empty(field: Term, value: Any) -> Criterion:
    """
...
def jsonb_operator_key_or(field: Term, value: Any) -> Criterion:
    """
    Поиск объектов с ключами из списка операция или
    """
    return JsonbFounder(field, field.wrap_constant(value), operation='?|')


def jsonb_operator_key_and(field: Term, value: Any) -> Criterion:
    """
    Поиск объектов с ключами из списка операция И
    """
    return JsonbFounder(field, field.wrap_constant(value), operation='?&')
...
def get_filters_for_field(
    ...
    return {
        ...
        # ++JSONB
        f"{field_name}__jsonb_include": {
            "field": actual_field_name,
            "source_field": source_field,
            "operator": jsonb_operator_include,
        },
        f"{field_name}__jsonb_match": {
            "field": actual_field_name,
            "source_field": source_field,
            "operator": jsonb_operator_match_or_empty,
        },
        f"{field_name}__jsonb_key_or": {
            "field": actual_field_name,
            "source_field": source_field,
            "operator": jsonb_operator_key_or,
        },
        f"{field_name}__jsonb_key_and": {
            "field": actual_field_name,
            "source_field": source_field,
            "operator": jsonb_operator_key_and,
        },
        # --JSONB
